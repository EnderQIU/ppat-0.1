...
from translators.pep.constants import PREDICTION_MODEL_WEIGHTS_PATH, MAX_CHAR_SEQ_LEN, MAX_PADDED_PHONE_SEQ_LEN, \
    START_PHONE_SYM, END_PHONE_SYM
from translators.pep.utils import PredictionModelUtils
...
class PredictionModel:
    ...
    def _build_model(self):
        ...
        def get_context(_encoder_outputs, h_prev):
            h_prev = attn_repeat(h_prev)
            concat = attn_concat([_encoder_outputs, h_prev])
            e = attn_dense1(concat)
            e = attn_dense2(e)
            attention_weights = attn_softmax(e)
            _context = attn_dot([attention_weights, _encoder_outputs])
            return _context
        ...
        testing_decoder_model = Model([test_prev_phone_input,
                                       test_h,
                                       test_c,
                                       test_encoding_input],
